...
const ConnectionPage = () => {
    ...
    useEffect(() => {
        setLoading(true);
        axios.get(`http://localhost:5050/connection/connections/${userId}`)
            .then(res => {
    ...
    useEffect(() => {
        setLoading(true);
        axios.get(`http://localhost:5050/connection/${userId}`)
            .then(res => {
    ...
    const handleAccept = (requestId) => {
        axios.patch(`http://localhost:5050/connection/${userId}`, { userID1: requestId })
            .then(() => {
    ...
    const handleReject = (requestId) => {
        axios.delete(`http://localhost:5050/connection/${userId}`, { data: { userID1: requestId } })
            .then(() => {
                setRequests(prev => prev.filter(req => req.userID1 !== requestId));
            })
            .catch(err => {
    ...
    const handleRemove = (connectionId, connectedUserId) => {
        axios.delete(`http://localhost:5050/connection/${connectedUserId}`, { data: { userID1: userId } })
            .then(() => {
                setConnections(prev => prev.filter(conn => conn.connectedUser._id !== connectedUserId));
            })
            .catch(err => {
                ...
                {requests.length === 0 ? (
                    ...
                    <List
                        ...
                        renderItem={item => (
                            ...
                            >
                                <List.Item.Meta
                                    avatar={<Avatar src={item.ProfilePic || undefined} icon={!item.ProfilePic ? <UserOutlined /> : undefined} />}
                                    title={
                                        <Button type="link" onClick={() => navigate(`/profilePage/${item.userID1}`)}>
                            ...
                            return (
                                <List.Item
                                    actions={[
                                        <Button 
                                            danger 
                                            icon={<DeleteOutlined />} 
                                            onClick={() => handleRemove(item._id, item.connectedUser._id)}
                                            shape="round"
                                        >
                                            Remove
                                        </Button>
                                ...
                                >
                                    <List.Item.Meta
                                        ...
                                        title={
                                            <Button type="link" onClick={() => navigate(`/profilePage/${item.userID1}`)}>
                                                {item.connectedUser ? `${item.connectedUser.FirstName} ${item.connectedUser.LastName}` : "User not found"}
                                            </Button>
                                        }
